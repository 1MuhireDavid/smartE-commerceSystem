...
public class ProductController {
    ...
    @GetMapping
    public ResponseEntity<org.example.api.dto.ApiResponse<PagedResponse<ProductEntity>>> list(
        ...
        return ResponseEntity.ok(
                org.example.api.dto.ApiResponse.success("Products retrieved successfully", data));
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<org.example.api.dto.ApiResponse<ProductEntity>> getById(
            @Parameter(description = "Product ID", required = true, example = "1")
            @PathVariable long id) {
        ProductEntity product = productService.findById(id);
        return ResponseEntity.ok(org.example.api.dto.ApiResponse.success(product));
    }
    ...
    @PostMapping
    public ResponseEntity<org.example.api.dto.ApiResponse<ProductEntity>> create(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Product details", required = true)
            @Valid @RequestBody ProductRequest request) {
        ProductEntity created = productService.create(request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(org.example.api.dto.ApiResponse.success("Product created successfully", created));
    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<org.example.api.dto.ApiResponse<ProductEntity>> update(
            @Parameter(description = "Product ID", required = true, example = "1")
            @PathVariable long id,
            @Valid @RequestBody ProductRequest request) {
        ProductEntity updated = productService.update(id, request);
        return ResponseEntity.ok(
                org.example.api.dto.ApiResponse.success("Product updated successfully", updated));
    }
    ...
    public ResponseEntity<org.example.api.dto.ApiResponse<Void>> delete(
        ...
        return ResponseEntity.ok(
                org.example.api.dto.ApiResponse.success("Product deleted successfully", null));
    }
}
