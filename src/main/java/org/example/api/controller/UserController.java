...
public class UserController {
    ...
    @GetMapping
    public ResponseEntity<org.example.api.dto.ApiResponse<PagedResponse<UserEntity>>> list(
        ...
        return ResponseEntity.ok(org.example.api.dto.ApiResponse.success("Users retrieved successfully", data));
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<org.example.api.dto.ApiResponse<UserEntity>> getById(
            @Parameter(description = "User ID", required = true, example = "1")
            @PathVariable long id) {
        UserEntity user = userService.findById(id);
        return ResponseEntity.ok(org.example.api.dto.ApiResponse.success(user));
    }
    ...
    @PostMapping
    public ResponseEntity<org.example.api.dto.ApiResponse<UserEntity>> create(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "User details", required = true)
            @Valid @RequestBody UserRequest request) {
        UserEntity created = userService.create(request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(org.example.api.dto.ApiResponse.success("User created successfully", created));
    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<org.example.api.dto.ApiResponse<UserEntity>> update(
            @Parameter(description = "User ID", required = true, example = "1")
            @PathVariable long id,
            @Valid @RequestBody UserRequest request) {
        UserEntity updated = userService.update(id, request);
        return ResponseEntity.ok(org.example.api.dto.ApiResponse.success("User updated successfully", updated));
    }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<org.example.api.dto.ApiResponse<Void>> delete(
            @Parameter(description = "User ID", required = true, example = "1")
            @PathVariable long id) {
        userService.delete(id);
        return ResponseEntity.ok(org.example.api.dto.ApiResponse.success("User deleted successfully", null));
    }
}
