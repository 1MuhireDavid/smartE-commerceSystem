...
public class CartController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponseDto<PagedResponse<CartEntity>>> list(
            @RequestParam(defaultValue = "0")  int page,
            @RequestParam(defaultValue = "20") int size) {
        int clampedSize = Math.min(Math.max(size, 1), 100);
        PagedResponse<CartEntity> data = cartService.findAll(PageRequest.of(page, clampedSize));
        return ResponseEntity.ok(ApiResponseDto.success("Carts retrieved successfully", data));
    }

    @Operation(summary = "Get cart by ID")
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponseDto<CartEntity>> getById(
            @Parameter(description = "Cart ID", example = "1") @PathVariable long id) {
        return ResponseEntity.ok(ApiResponseDto.success(cartService.findById(id)));
    }

    @Operation(summary = "Get active cart for a user")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponseDto<CartEntity>> getActiveByUser(
            @Parameter(description = "User ID", example = "1") @PathVariable long userId) {
        return ResponseEntity.ok(ApiResponseDto.success(cartService.findActiveByUserId(userId)));
    }

    @Operation(summary = "Create a new cart for a user")
    @PostMapping
    public ResponseEntity<ApiResponseDto<CartEntity>> create(@Valid @RequestBody CartRequest request) {
        CartEntity created = cartService.create(request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponseDto.success("Cart created successfully", created));
    }

    @Operation(summary = "List items in a cart")
    @GetMapping("/{id}/items")
    public ResponseEntity<ApiResponseDto<List<CartItemEntity>>> getItems(
            @Parameter(description = "Cart ID", example = "1") @PathVariable long id) {
        return ResponseEntity.ok(ApiResponseDto.success(cartService.getItems(id)));
    }
    ...
    public ResponseEntity<ApiResponseDto<CartItemEntity>> addItem(
            @Parameter(description = "Cart ID", example = "1") @PathVariable long id,
            @Valid @RequestBody CartItemRequest request) {
        CartItemEntity item = cartService.addItem(id, request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponseDto.success("Item added to cart", item));
    }

    @Operation(summary = "Update the quantity of a cart item")
    @PutMapping("/{id}/items/{itemId}")
    public ResponseEntity<ApiResponseDto<CartItemEntity>> updateItem(
            @Parameter(description = "Cart ID", example = "1")     @PathVariable long id,
        ...
        return ResponseEntity.ok(ApiResponseDto.success("Cart item updated", updated));
    }
